...
| 模块       | 能力                                                                                                             |
| ---------- | ---------------------------------------------------------------------------------------------------------------- |
| 工作区     | 扫描一级项目、按可配置深度识别子项目、维护忽略目录与备注、查看 Git 状态、绑定并验证 Git 身份、手动纳入外部项目   |
...
| Git 配置   | 管理全局配置和多身份 profile，根据工作区生成 `includeIf` 规则，并在真实仓库验证最终生效值与来源                  |
| SSH 密钥   | 发现现有公钥、生成 ed25519 / RSA 4096 密钥、查看指纹，并在删除前确认关联影响                                     |
| Node 管理  | 支持官方源、npmmirror 和自定义下载地址，通过 nvm 安装、切换和设置默认版本；管理 nrm 镜像、包管理器、全局包和缓存 |
| Hosts      | 维护独立受管区块，显式预览并导入系统映射，支持启停、备份恢复、打开 hosts 文件、刷新 DNS 和访问配置域名           |
...
macOS Release 工作流已接入 Developer ID 签名与 Apple 公证。仓库需要配置 `MAC_CERTIFICATE`、`MAC_CERTIFICATE_PASSWORD`、`APPLE_API_KEY_BASE64`、`APPLE_API_KEY_ID` 和 `APPLE_API_ISSUER` Secrets；其中 API Key 使用 `.p8` 文件的 Base64 内容。缺少凭据时 macOS 正式发布会失败，避免发布无法可靠更新的未签名构建。
...
自动发布使用仓库自带的 `GITHUB_TOKEN`。流水线会先执行 lint、类型检查、测试和构建入口检查，全部通过后才开始三平台打包。

Node 自定义下载源默认只接受 HTTPS；仅 `localhost`、`127.0.0.1` 和 `::1` 可使用 HTTP。设置抽屉可以测试版本索引、目标安装包和校验文件。安装过程会校验镜像提供的 SHA256，但同源校验不能证明自定义镜像发布者可信，使用自定义源前仍需确认来源。
...
- 数据 schema 当前为 v2，导入 v1 备份时会自动迁移设置、工作区扫描配置和首页趋势历史。
